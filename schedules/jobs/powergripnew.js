...
module.exports = async ({client, category}) => {
    ...
    const products = doc('#mpc_uutuudet .col-6').toArray().map(elem => {
        try{
            ...
            const available = doc(elem).hasClass('text-danger');
            return {
                name: `${brand} - ${name}`,
                id: `${brand} - ${name}`,
                available,
            ...
            console.log(ex)
            log(`[DG-SCHEDULER]: Failed checking new powergrip product ${ex.message}`, 3)
        }
    ...
    for(const product of products){
        ...
        const discordMessage = new MessageEmbed()
            .setTitle(`${product.name} havaittu powergripissä`)
        ...
        activeSubscriptions.forEach(async (subscription) => {
            ...
            await client.db.saveSentMessage({
                content: `${product.name} havaittu powergripissä`,
                messageId: sent.id,
